...
#include "directoryeditorplugin.h"
...
using namespace DIRECTORYEditor::Internal;
...
//};
 ...
 DirectoryEditorFile::DirectoryEditorFile(DirectoryEditor *parent) :
        Core::IFile(parent),
        m_mimeType(QLatin1String(DIRECTORYEditor::Constants::C_DIRECTORYEDITOR_MIMETYPE))
    {
        m_editor = parent;
        //connect(m_editor, SIGNAL(lazyDataRequested(int, bool)), this, SLOT(provideData(int)));
    }
    DirectoryEditorFile::~DirectoryEditorFile() {}
...
//        }
    ...
    void DirectoryEditorFile::provideData(int block) {
...
//        }
    ...
    void DirectoryEditorFile::setFilename(const QString &filename) {
        m_fileName = filename;
    }

    QString DirectoryEditorFile::fileName() const {
        return m_fileName;
    }

    QString DirectoryEditorFile::defaultPath() const { return QString(); }
    QString DirectoryEditorFile::suggestedFileName() const { return QString(); }
    QString DirectoryEditorFile::fileFilter() const { return QString(); }
    QString DirectoryEditorFile::fileExtension() const { return QString(); }

    bool DirectoryEditorFile::isModified() const {
        return false;/*m_editor->isModified()*/;
    }
    bool DirectoryEditorFile::isReadOnly() const {
        //const QFileInfo fi(m_fileName);
        return false;//!fi.isWritable();
    }

    bool DirectoryEditorFile::isSaveAsAllowed() const { return true; }

    void DirectoryEditorFile::modified(ReloadBehavior *behavior) {
...
//        }
 ...
 DirectoryEditorInterface::DirectoryEditorInterface(DirectoryEditor *parent)
        : Core::IEditor(parent)
    {
        Core::UniqueIDManager *uidm = Core::UniqueIDManager::instance();
        m_editor = parent;
        m_file = new DirectoryEditorFile(parent);
//        m_context << uidm->uniqueIdentifier(Core::Constants::K_DEFAULT_BINARY_EDITOR);
        m_context << uidm->uniqueIdentifier(Constants::C_DIRECTORYEDITOR);
...
//        connect(m_editor, SIGNAL(cursorPositionChanged(int)), this, SLOT(updateCursorPosition(int)));
    ...
    DirectoryEditorInterface::~DirectoryEditorInterface() {}

    QWidget *DirectoryEditorInterface::widget() { return m_editor; }

    QList<int> DirectoryEditorInterface::context() const { return m_context; }

    bool DirectoryEditorInterface::createNew(const QString & /* contents */ ) {
        m_file->setFilename(QString());
        return true;
    }
    bool DirectoryEditorInterface::open(const QString &fileName ) {
        return true;/*m_file->open(fileName)*/;
    }
    Core::IFile *DirectoryEditorInterface::file() { return m_file; }
    const char* DirectoryEditorInterface::kind() const
    { return DIRECTORYEditor::Constants::C_DIRECTORYEDITOR; }
    QString DirectoryEditorInterface::displayName() const { return m_displayName; }
    void DirectoryEditorInterface::setDisplayName(const QString &title) { m_displayName = title; emit changed(); }

    bool DirectoryEditorInterface::duplicateSupported() const { return false; }
    Core::IEditor *DirectoryEditorInterface::duplicate(QWidget* )
    {   return 0; }

    QByteArray DirectoryEditorInterface::saveState() const { return QByteArray(); } // TODO
    bool DirectoryEditorInterface::restoreState(const QByteArray & /* state */) { return false; } // TODO

    QWidget *DirectoryEditorInterface::toolBar() { return 0 /*m_toolBar*/; }

    bool DirectoryEditorInterface::isTemporary() const { return true; }

///////////////////////////////// DirectoryEditorFactory //////////////////////////////////

DirectoryEditorFactory::DirectoryEditorFactory(DirectoryEditorPlugin *owner) :
    m_kind(QLatin1String(DIRECTORYEditor::Constants::C_DIRECTORYEDITOR)),
    m_mimeTypes(QLatin1String(DIRECTORYEditor::Constants::C_DIRECTORYEDITOR_MIMETYPE)),
    m_owner(owner)
{
}

QString DirectoryEditorFactory::kind() const
{
    return m_kind;
}

Core::IFile *DirectoryEditorFactory::open(const QString &fileName)
{
...
Core::IEditor *DirectoryEditorFactory::createEditor(QWidget *parent)
{
    DirectoryEditor *editor = new DirectoryEditor(parent);
    m_owner->initializeEditor(editor);
    return editor->editorInterface();
}

QStringList DirectoryEditorFactory::mimeTypes() const
{
    return m_mimeTypes;
}
...
DirectoryEditorPlugin::DirectoryEditorPlugin()
{
...
QAction *DirectoryEditorPlugin::registerNewAction(const QString &id, const QString &title)
{
...
QAction *DirectoryEditorPlugin::registerNewAction(const QString &id,
                                            QObject *receiver,
                                            const char *slot,
                                            const QString &title)
{
...
void DirectoryEditorPlugin::initializeEditor(DirectoryEditor *editor)
{
    DirectoryEditorInterface *editorInterface = new DirectoryEditorInterface(editor);
...
bool DirectoryEditorPlugin::initialize(const QStringList &arguments, QString *errorMessage)
{
    Q_UNUSED(arguments)
    ...
    addAutoReleasedObject(new DirectoryEditorFactory(this));

    return true;
}

void DirectoryEditorPlugin::extensionsInitialized()
{
}

void DirectoryEditorPlugin::updateCurrentEditor(Core::IContext *object)
{
    do {
        if (!object) {
            if (!m_currentEditor)
        ...
        DirectoryEditor *editor = qobject_cast<DirectoryEditor *>(object->widget());
        if (!editor) {
            if (!m_currentEditor)
                return;
...
void DirectoryEditorPlugin::updateActions()
{
...
void DirectoryEditorPlugin::undoAction()
{
//    if (m_currentEditor)
//        m_currentEditor->undo();
}

void DirectoryEditorPlugin::redoAction()
{
//    if (m_currentEditor)
//        m_currentEditor->redo();
}

void DirectoryEditorPlugin::copyAction()
{
//    if (m_currentEditor)
//        m_currentEditor->copy();
}

void DirectoryEditorPlugin::selectAllAction()
{
//    if (m_currentEditor)
...
Q_EXPORT_PLUGIN(DirectoryEditorPlugin)

//#include "directoryeditorplugin.moc"
